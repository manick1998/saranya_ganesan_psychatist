...
(function () {
  ...
  function renderFooter() {
    ...
    const so = $("#socials"); if (so) so.innerHTML = socials.map(function (s) { var ext = s.href.indexOf("http") === 0; return '<a href="' + s.href + '"' + (ext ? ' target="_blank" rel="noopener noreferrer"' : '') + ' aria-label="Follow on ' + s.label + '">' + s.label + '</a>'; }).join("");
